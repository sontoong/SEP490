...
saleApi.interceptors.request.use(async (config) => {
...
const saleRequests = {
  get: <T>(url: string, params?: T) =>
    saleApi.get(url, { params }).then(responseBody),
  post: <T>(url: string, body: T) => saleApi.post(url, body).then(responseBody),
  put: <T>(url: string, body: T) => saleApi.put(url, body).then(responseBody),
  patch: <T>(url: string, body: T) =>
    saleApi.patch(url, body).then(responseBody),
  del: <T>(url: string, params?: T) =>
    saleApi.delete(url, { params }).then(responseBody),
};

const baseSaleRequests = baseRequests(baseURL);

export { saleRequests, baseSaleRequests };
